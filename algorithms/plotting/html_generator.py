...
import os
import datetime
from algorithms.data.loader import slots, activities_dict, groups_dict, spaces_dict, lecturers_dict
...
def _generate_timetable_row(time_range, time_slots, timetable, group_id):
    ...
    for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]:
        ...
        if day in time_slots[time_range]:
            ...
            # The timetable is now a dictionary: Dict[slot_id: int, Dict[room_id: str, Activity | None]]
            # Iterate through the structure correctly.
            if slot in timetable: # Check if the slot exists in the timetable
                room_dict = timetable[slot]
                for room_id, activity in room_dict.items():
                    if activity is not None and group_id in activity.group_ids:
                        row_html += format_activity_html(activity, room_id) # Pass room_id
                        group_activities_found = True
                        break # Found activity for this group in this slot, move to next day
        ...
        for slot in slots_dict:
            if slot_count < 5:  # Just show a few samples
                ...
                for room in slots_dict[slot]:
                    activity = timetable[slot].get(room)
