...
from algorithms.ga.nsga2 import mutate, crossover
import math
import copy
        ...
        for activity in unassigned_activities:
            ...
            if feasible_slots:
                ...
                for slot in feasible_slots:
                    ...
                    if suitable_rooms:
                        ...
                        timetable[slot][room] = activity  # Store the actual activity object
                        assigned = True
                        break
            ...
            if not assigned:
                ...
                if best_slot is not None:
                    timetable[best_slot][best_room] = activity  # Store the actual activity object
...
def run_spea2_optimizer(population_size=None, num_generations=None, archive_size=20, output_dir="spea2_output", enable_plotting=False):
    """
    ...
    Parameters:
        ...
        num_generations: Number of generations to run (default: uses global NUM_GENERATIONS)
        archive_size: Size of the archive (default: uses global ARCHIVE_SIZE or 20)
    ...
    ARCHIVE_SIZE = archive_size if archive_size is not None else ARCHIVE_SIZE # Use provided or global
    ...
    for generation in range(NUM_GENERATIONS):
        ...
        if generation == NUM_GENERATIONS - 1:
            # Add the final Pareto front points to the metrics
            metrics["final_pareto_front"] = archive_fitness
...
def evaluate_individual(individual, evaluator_instance=None):
    ...
    for slot in individual:
        ...
        for room in individual[slot]:
            activity = individual[slot][room]
    ...
    # Calculate weighted hard constraints similar to NSGA-II for consistent comparison
    hard_violations = (
    ...
    # TODO: Implement a proper soft constraints evaluation
    soft_score = 0.5
...
def find_violations(timetable):
    ...
    for slot in slots:
        ...
        for room, activity in timetable[slot].items():
            ...
            # Only process if this is an activity object with expected attributes
            if not hasattr(activity, 'id') or not hasattr(activity, 'teacher_id') or not hasattr(activity, 'group_ids'):
                continue
...
def repair_mutation(timetable):
    ...
    for slot, room, activity in violations:
        if activity is None:
            continue
        ...
        for new_slot in slots:
            for new_room in spaces_dict.keys():
                ...
                individual[new_slot][new_room] = activity
                
                # Check if this creates violations
                new_violations = find_violations(individual)
                if not any((s == new_slot and r == new_room) for s, r, _ in new_violations):
        ...
        for activity, _, _ in sorted_unassigned:
            ...
            if feasible_slots:
                ...
                for slot in feasible_slots:
                    ...
                    if suitable_rooms:
                        room = random.choice(suitable_rooms)
                        individual[slot][room] = activity  # Store the activity object directly
                        assigned = True
                        break
            ...
            if not assigned:
                ...
                if best_slot is not None and best_room is not None:
                    individual[best_slot][best_room] = activity  # Store the activity object directly
