...
from algorithms.evaluation.evaluator import evaluate_hard_constraints, evaluate_soft_constraints
...
def run_sarsa_optimizer(activities_dict, groups_dict, spaces_dict, lecturers_dict, slots, learning_rate=0.001, episodes=100, epsilon=0.1):
    ...
    for epoch in range(episodes):
        ...
        for idx, activity in activities_to_assign:
            ...
            if activities_to_assign.index((idx, activity)) < len(activities_to_assign) - 1 and next_available:
                next_idx, _ = activities_to_assign[activities_to_assign.index((idx, activity)) + 1]
        ...
        hard_violations = evaluate_hard_constraints(current_schedule, activities_dict, groups_dict, spaces_dict)
        _, soft_score = evaluate_soft_constraints(current_schedule, groups_dict, lecturers_dict, slots)
    ...
    if best_schedule:
        ...
        hard_violations_tuple = evaluate_hard_constraints(final_schedule, activities_dict, groups_dict, spaces_dict)
        _, final_soft_score = evaluate_soft_constraints(final_schedule, groups_dict, lecturers_dict, slots)
